...
CREATE TABLE `comment` (
  ...
  CONSTRAINT `fk_Komentar_Korisnik1` FOREIGN KEY (`idUser`) REFERENCES `user` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_Komentar_Objava1` FOREIGN KEY (`idPost`) REFERENCES `post` (`id`) ON DELETE CASCADE
...
CREATE TABLE `follower` (
  ...
  CONSTRAINT `fk_Pratitelji_Korisnik1` FOREIGN KEY (`idFollower`) REFERENCES `user` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_Pratitelji_Korisnik2` FOREIGN KEY (`idFollowed`) REFERENCES `user` (`id`) ON DELETE CASCADE
...
CREATE TABLE `post` (
  ...
  CONSTRAINT `fk_Objava_Korisnik` FOREIGN KEY (`idUser`) REFERENCES `user` (`id`) ON DELETE CASCADE
...
CREATE TABLE `rating` (
  ...
  CONSTRAINT `fk_ObjavaLike_Objava1` FOREIGN KEY (`idPost`) REFERENCES `post` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_ObjavaLikeDislike_Korisnik1` FOREIGN KEY (`idUser`) REFERENCES `user` (`id`) ON DELETE CASCADE
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `DeleteUser`(id_ INT)
BEGIN
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `EditProfile`(id_ INT, username_ varchar(45), name_ varchar(45), surname_ varchar(45), dateOfBirth_ DATE)
BEGIN
	UPDATE user
	SET Username = username_, Name = name_, Surname = surname_, DateOfBirth = dateOfBirth_
	WHERE id = id_;
    
END */$$
DELIMITER ;

/* Procedure structure for procedure `EditUser` */

/*!50003 DROP PROCEDURE IF EXISTS  `EditUser` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `EditUser`(id_ INT, Username_ VARCHAR(45), Name_ VARCHAR(45), Surname_ VARCHAR(45), DateOfBirth_ DATE, IsAdmin_ INT)
BEGIN
   UPDATE user
   SET Username=Username_, Name=Name_, Surname=Surname_, DateOfBirth=DateOfBirth_, IsAdmin=IsAdmin_
   WHERE id=id_;
END */$$
DELIMITER ;
...
BEGIN
    SELECT id, Username as username, NAme as name, Surname as surname, DateOfBirth as dateOfBirth, IsAdmin as isAdmin FROM user;
END */$$
DELIMITER ;
