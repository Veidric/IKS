CREATE DATABASE  IF NOT EXISTS `iks` /*!40100 DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_bin */ /*!80016 DEFAULT ENCRYPTION='N' */;
USE `iks`;
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `DeleteUser`(id_ INT)
BEGIN
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `EditUser`(id_ INT, Username_ VARCHAR(50), Name_ VARCHAR(50), Surname_ VARCHAR(50), DateOfBirth_ DATE, IsAdmin_ INT)
BEGIN
   UPDATE user
...
BEGIN
    SELECT id, Username, NAme, Surname, DateOfBirth, IsAdmin FROM user;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
/*!50003 SET sql_mode              = 'NO_AUTO_VALUE_ON_ZERO' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `MakeChat`(in idParticipant1_ INT, in idParticipant2_ INT)
BEGIN
	declare chatId int;
...
-- Dump completed on 2025-12-03 16:34:11
